...
export const deleteThought = async (req, res) => {
    try {
        ...
        const user = await User.findOneAndUpdate({ thoughts: req.params.thoughtId }, { $pull: { applications: req.params.applicationId } }, { new: true });
        if (!user) {
            return res.status(404).json({
...
export const addReaction = async (req, res) => {
    try {
        const reaction = await reactionThought.findOneAndUpdate({ _id: req.params.thoughtId }, { $addToSet: { tags: req.body } }, { runValidators: true, new: true });
        if (!reaction) {
            return res.status(404).json({ message: 'No thought with this id!' });
        }
        res.json(reaction);
...
export const removeReaction = async (req, res) => {
    try {
        const reaction = await reactionThought.findOneAndUpdate({ _id: req.params.reactionId }, { $addToSet: { reactions: req.body } }, { runValidators: true, new: true });
        if (!reaction) {
